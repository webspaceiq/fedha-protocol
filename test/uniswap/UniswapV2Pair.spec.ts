...
import { UniswapV2Pair, FedhaERC20Token } from "../../typechain-types";
...
describe("UniswapV2Pair", () => {
  async function fixture() {
    ...
    const tokenA = (await (
      await ethers.getContractFactory("FedhaERC20Token")
    ).deploy("Naira C", "NGNC", expandTo18Decimals(10000))) as FedhaERC20Token;
    const tokenB = (await (
      await ethers.getContractFactory("FedhaERC20Token")
    ).deploy("Naira C", "NGNC", expandTo18Decimals(10000))) as FedhaERC20Token;
  ...
  async function addLiquidity(
    token0: FedhaERC20Token,
    token1: FedhaERC20Token,
    pair: UniswapV2Pair,
  ...
  it("swap:gas", async () => {
    ...
    expect(receipt.gasUsed).to.eq(77723);
  });
